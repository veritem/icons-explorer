...
async function prepareJSON() {
	...
	const infoOut = path.resolve(__dirname, '../src/lib/data');
...
async function copyLibs() {
	...
	await fs.copy(path.join(modules, '@iconify/iconify/dist/'), path.join(out, 'lib'), {
		filter: (src) => {
	...
	await fs.copy(path.join(modules, 'jszip/dist/jszip.min.js'), path.join(out, 'lib/jszip.min.js'));
}
